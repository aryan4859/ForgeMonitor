...
const { data: urls } = await useAsyncData("urls", () =>
  queryContent("/urls").find()
);
const { data: logs } = await useLazyAsyncData("logs", () =>
  queryContent("/logs").find()
);
...
<template>
  <div class="">
    ...
    <div class="flex flex-col items-center">
      <Card
        :meta_data="url"
        :report_data="retrieveLogs(url._path)"
        v-for="url in urls"
        :key="url._id"
      ></Card>
    </div>
